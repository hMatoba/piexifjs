...
test('load returns a object contains IFD - 1', () => {
  const jpegBinary = fs.readFileSync("./node_tests/files/r_canon.jpg").toString("binary");
  const exifObj = piexifjs.load(jpegBinary);
  expect(Object.keys(exifObj)).toContain('0th');
});

test('round trip "load" and "dump" - 1', () => {
  const jpegBinary = fs.readFileSync("./node_tests/files/r_sony.jpg").toString("binary");
...
test('success remove - 1', () => {
  const jpegBinary = fs.readFileSync("./node_tests/files/r_pana.jpg").toString("binary");
  const exifObj = piexifjs.load(jpegBinary);
  expect(Object.keys(exifObj)).toContain('0th');
...
test('success insert - 1', () => {
  const jpegBinary = fs.readFileSync("./node_tests/files/noexif.jpg").toString("binary");
  const exifObj = piexifjs.load(jpegBinary);
